...
async function send_request_templates_for_file(file_name) {
  ...
  const r = await api.fetchApi("/itools/request_templates_for_file", {
    method: "POST",
    body,
  });
  return r.json();
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      if (allow_debug) {
        console.log("iToolsPromptStylerExtra", this);
      }
...
async function send_style_message_multi(base, s1, s2, s3) {
  ...
  const respond = await api.fetchApi("/itools/style_change_multi", {
    method: "POST",
    request_body,
  });
  return respond.json();
